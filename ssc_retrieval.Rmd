...
reverse_scale_flow <- function(.x) {
  ...
  foo <<- in_situ_flow
  # Case 1: no flow measurements at that day
  ...
  .x[.x$in_situ_flow==F,]$flow_m3s = rescale(.x[.x$in_situ_flow==F,]$flow_m3s_scale, 
                                to = c(0, in_situ_flow[in_situ_flow$distance_km == max(in_situ_flow$distance_km),]$flow_m3s[1]))
  .x$reverse_scaled = T
  return(.x)
}

# To compare SHAPE of curve, scale flow data to 0-1
landsat_flux$flow_m3s_scale <- -1
...
landsat_flux[landsat_flux$dead==T, ]$flow_m3s_scale <- 
  predict(flowvdrainage_preseminole, landsat_flux[landsat_flux$dead==T, ])
