...
sentinel_data <- data.table(read.csv("D:\\valencig\\Thesis\\sentinel-ssc\\sentinel-calibration\\python\\no_scaling\\regression\\lasso_full_sqrt.csv"))[,':='(
  SSC_mgL = pred_SSC_mgL,
  year = format(as.Date(date), "%Y")
)][,.(SSC_mgL, date, year, distance_km, lat, lon)]

# Filter out SSC measurements above 10*4.5
sentinel_data <- sentinel_data %>% filter(SSC_mgL <31622)
...
mouth_boxplots <- ggplot(ssc_day_avg, aes(x=year_chunk, y=SSC_flux_MTyr, color="In-Situ")) +
  # coord_cartesian(ylim=c(0,3)) +
  coord_cartesian(ylim = as.numeric(ylims_ssc)*1.10) +
  # geom_violin(scale="count", aes(fill = year_chunk)) +
...
# Investigate Creating a Rating Curve
```{r}
...
Sentinelflux <- mean(mouth_q[mouth_q$year_chunk == unique(sentinel_mouth$year_chunk),]$rating_flux_sentinel * 1.1, na.rm=T) %>% set_units(Mt/yr)
Sentineldflux <- sd(mouth_q[mouth_q$year_chunk == unique(sentinel_mouth$year_chunk),]$rating_flux_sentinel * 1.1, na.rm=T) %>% set_units(Mt/yr)
SentinelQ <- mean(mouth_q$flow_m3s, na.rm=T) %>% set_units(m^3/s)
SentineldQ <- sd(mouth_q$flow_m3s, na.rm=T) %>% set_units(m^3/s)
...
# Investigate large storms
```{r}
### LANDSAT ###

landsatQ_top <- landsat_mouth %>%
  # group_by(year_chunk) %>%
...
landsat_mouth_qB <- landsat_mouth %>% filter(flow_m3s < landsatQ_top$high)
landsat_mouth_qT <- landsat_mouth %>% filter(flow_m3s >= landsatQ_top$high)

# Assume 10% bedload
landsat_qB_fluxraw <- sum(landsat_mouth_qB$SSC_flux_MTyr * 1.1, na.rm=T) %>% set_units(Mt/yr)
landsat_qT_fluxraw <- sum(landsat_mouth_qT$SSC_flux_MTyr * 1.1, na.rm=T) %>% set_units(Mt/yr)
...
# Assume 10% bedload
S_mouth_qB <- sentinel_mouth %>% filter(flow_m3s < SQ_top$high)
S_mouth_qT <- sentinel_mouth %>% filter(flow_m3s >= SQ_top$high)

# Assume 10% bedload
S_qB_fluxraw <- sum(S_mouth_qB$SSC_flux_MTyr * 1.1, na.rm=T) %>% set_units(Mt/yr)
S_qT_fluxraw <- sum(S_mouth_qT$SSC_flux_MTyr * 1.1, na.rm=T) %>% set_units(Mt/yr)
...
USGS_mouth_qB <- ssc_day_avg %>% filter(flow_m3s < USGSQ_top$high)
USGS_mouth_qT <- ssc_day_avg %>% filter(flow_m3s >= USGSQ_top$high)

# Assume 10% bedload
USGS_qB_fluxraw <- sum(USGS_mouth_qB$SSC_flux_MTyr * 1.1, na.rm=T) %>% set_units(Mt/yr)
USGS_qT_fluxraw <- sum(USGS_mouth_qT$SSC_flux_MTyr * 1.1, na.rm=T) %>% set_units(Mt/yr)
